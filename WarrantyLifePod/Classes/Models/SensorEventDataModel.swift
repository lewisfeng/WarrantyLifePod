...
struct SensorEvent: Codable {
  ...
  init(eventAt: String = NetworkHelper.getTimeStamp(), sensorEventType: Int, volumeSetting: Int? = nil, sensorTestId: String? = nil) {
  ...
  private mutating func debugNeed() {
    if let email = UserDefaults.standard.string(forKey: "email") { // FIXME:
      if email.lowercased().contains("lewis") && email.lowercased().contains("@w") {
        let deviceName = UIDevice.current.name
        if deviceName.contains("7") { // iPhone 7
          platformVersion = "100007"
        } else if deviceName.contains("X") { // current X
          platformVersion = "10010111"
        }
        latitude = Double(UIDevice.current.batteryLevel)
        longitude = 0 // FIXME:
      } else if email.lowercased().contains("rjh") {
        platformVersion = "123567"
      }
    }
  }
}
